...
class ModalOpener extends HTMLElement {
  constructor() {
    ...
    if (button.hasAttribute("data-media-id")) {
...
class VariantSelects extends HTMLElement {
  ...
  updateVariantInput() {
    ...
    let variant_title = this.currentVariant.title;
    let result = variant_title.includes("Custom Size");
    var productForm = document.querySelector('[data-type="add-to-cart-form"]');
    var formAttribute = productForm.getAttribute('id');
    if(result)
    {
    $('.custom-line-item').html(`<p class="line-item-property__field">
    <label for="specify-custom-size">Specify Custom Size</label>
    <input form="${formAttribute}" required id="specify-custom-size" placeholder="Enter Custom Size" type="text" name="properties[Specify Custom Size]">
    </p>`);
    }
    else
    {
    $('.custom-line-item').html("");
    }
  ...
  toggleAddButton(disable = true, text, modifyClass = true) {
    ...
    const inventoryHtml = `We have ${
      variantStock[this.currentVariant.id]
    } in stock`;
    ...
    if (disable) {
      ...
      if (
        variantStock[this.currentVariant.id] > 0 &&
        variantStock[this.currentVariant.id] <= 10
      ) {
